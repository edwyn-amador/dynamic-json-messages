const DJM = (() => {
  ...
  const applyDebounce = callback => {
    let timeout;
    return async (key = "", { waitTime = 300, response = "", excludeTemplate, lock = false, reset = false } = {}) => {
      clearTimeout(timeout);
      return new Promise(resolve => {
        timeout = setTimeout(() => resolve(callback(key, { response, excludeTemplate, lock, reset })), waitTime);
      });
    };
  };
        ...
        [
          `onmessage=${e => {
            ...
            setInstance(e.data, data => {
              getResult(data.key, data.response, data.excludeTemplate, data.canAppendResponses);
            });
            function getResult(key, response, excludeTemplate, appendResponses) {
              ...
              if (savedInfo.allNodesExcluded) {
                ...
                postMessage({ type: "recall", data: djm });
                ...
                try {
                  ...
                  if (!dataAttribs["*DJM_shown"]) {
                    ...
                    if (jsonStringKey.length === 0) {
                      ...
                      } else if (!savedInfo.allNodesExcluded) sendMessage("recall");
  ...
  const _Run = async (key = "", { response = "", excludeTemplate, lock = false, reset = false } = {}) => {
    if (context.isContentScript)
      return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({ DJM_Run: true, data: { key, response, excludeTemplate, lock, reset } }, result => {
        ...
        mainWorker.postMessage(
          keyInstance === undefined
            ? {
                ...
                reloaded: jsonString[key].length === 1,
                response,
                excludeTemplate,
                canAppendResponses
              }
  ...
  if (context.isChromeExt)
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.DJM_IsJSONFileLoaded) sendResponse(jsonString !== "");
        ...
        const { key, response, excludeTemplate, lock } = request.data;
        _Run(key, { response, excludeTemplate, lock })
