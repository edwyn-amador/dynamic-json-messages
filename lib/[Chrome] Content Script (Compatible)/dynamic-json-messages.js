const DJM = (() => {
  ...
  const _Settings = ({ isRefreshable = true, appendResponses = false, replaceOnWorker = false, mergeOnWorker = false }) => {
    if (context.isContentScript) {
      chrome.runtime.sendMessage({ DJM_Settings: true, data: { isRefreshable, appendResponses, replaceOnWorker, mergeOnWorker } });
      return;
    }
                ...
                try {
                  ...
                  if (
                    ...
                    if (!dataAttribs["*DJM_shown"]) {
                      savedInfo.isAlternate = !!+alternateProperty.value;
                      dataAttribs["*DJM_shown"] = true;
                    }
    ...
    if (context.isContentScript)
      return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage(
          { DJM_Run: true, data: { key, response, replaceTemplate, excludeTemplate, mergeTemplate, lock, reset } },
          result => {
  ...
  if (context.isChromeExt)
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.DJM_IsJSONFileLoaded) sendResponse(jsonString !== "");
        ...
        const { isRefreshable, appendResponses, replaceOnWorker, mergeOnWorker } = request.data;
        _Settings({ isRefreshable, appendResponses, replaceOnWorker, mergeOnWorker });
      } else if (request.DJM_Run) {
        const { key, response, replaceTemplate, excludeTemplate, mergeTemplate, lock, reset } = request.data;
        _Run(key, { response, replaceTemplate, excludeTemplate, mergeTemplate, lock, reset })
